...
const fs = require('fs');
const { AZURE_STORAGE_ACCOUNT } = require('../common/const');
  ...
  self._clientManager.getRepositoryInfo(options, (error, info) => {
    ...
    self._blobSvc.createBlockBlobFromStream(
      ...
      options, (uploadError, uploadResult, uploadResponse) => {
        ...
        // Adiciona URL de acesso ao objeto de resposta de upload
        const { container, name } = uploadResult;
        const cdnFileUrl = `https://${AZURE_STORAGE_ACCOUNT}.azureedge.net/${container}/${name}`;
        const fileUrl = `https://${AZURE_STORAGE_ACCOUNT}.blob.core.windows.net/${container}/${name}`;

        return callback(null, assign({}, uploadResult, {
          cdnFileUrl,
          fileUrl,
        }));
      });
...
StorageManager.prototype.delete = function (
  ...
  self._clientManager.getRepositoryInfo(options, (error, info) => {
    ...
    // Realizando a remoção do arquivo.
    self._blobSvc.deleteBlob(
      containerInfo.mediaStore, blobName, options,
      (getError, result) => {
        if (getError && !result.isSuccessful) {
          err = {
            statusCode: error.statusCode,
